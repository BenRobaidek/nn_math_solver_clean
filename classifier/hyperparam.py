            ...
            if not (embfix and not ptemb):
                ...
                if not hyperparams in list(hyperparam_results.keys()):
                    ...
                    try:
                        ...
                        s2s_validation_predictions_path = config['data_path'].strip('/working/basic/') + 'a/output/s2s/corrects_valk1234.tsv'
                        s2s_test_predictions_path = config['data_path'].strip('/working/basic/') + 'a/output/s2s/corrects_testk5.tsv'
                        ...
                        print('np.sum(s2s_test_predictions.astype(int))', np.sum(s2s_test_predictions.astype(int)))
                        print('len(s2s_test_predictions.astype(int))', len(s2s_test_predictions.astype(int)))
